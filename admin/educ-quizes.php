...
function logQuizAction($con, $action_type, $subject_name, $quiz_level, $quiz_id, $question_text, $old_values = null, $new_values = null, $changed_fields = null) {
    ...
    $performed_by_id = $_SESSION['id'] ?? 0;
    $performed_by_name = mysqli_real_escape_string($con, $_SESSION['name'] ?? 'Admin');
...
$current_subject = $_GET['subject'] ?? ($all_subjects[0] ?? '');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['import_csv'])) {
        ...
        foreach ($data_rows as $row) {
            ...
            $q = "INSERT INTO quizes (subject_name, quiz_level, category, question, answer_a, answer_b, answer_c, answer_d, correct_answer_number)
                  VALUES ('$row_subject', '$row_level', '$row_category', '$row_question', '$row_a', '$row_b', '$row_c', '$row_d', '$row_correct')";
            ...
            if (mysqli_query($con, $q)) {
                ...
                logQuizAction($con, 'ADD', $row_subject, $row_level, $new_id, $row_question, null,
                    ['subject_name'=>$row_subject,'quiz_level'=>$row_level,'category'=>$row_category,'question'=>$row_question,
                     'answer_a'=>$row_a,'answer_b'=>$row_b,'answer_c'=>$row_c,'answer_d'=>$row_d,'correct_answer_number'=>$row_correct]);
                $imported++;
            } else {
                $skipped++;
            }
    ...
    if (isset($_POST['delete_all_quiz'])) {
        $subject_name = mysqli_real_escape_string($con, $_POST['subject_name']);
        $level = mysqli_real_escape_string($con, $_POST['level']);

        // Get all quizzes that will be deleted for logging summary
        $select_query = "SELECT * FROM quizes WHERE subject_name = '$subject_name'";
        if ($level !== 'all') {
            $select_query .= " AND quiz_level = '$level'";
        }
        
        $quizzes_to_delete = mysqli_query($con, $select_query);
        $deleted_count = mysqli_num_rows($quizzes_to_delete);
        
        // Base delete query
        $query = "DELETE FROM quizes WHERE subject_name = '$subject_name'";
        if ($level !== 'all') {
            $query .= " AND quiz_level = '$level'";
        }
        ...
        if (mysqli_query($con, $query)) {
            ...
            logQuizAction(
                ...
                null, 
                "Bulk delete of $deleted_count questions for " . ($level !== 'all' ? "Level $level" : "all levels"),
                ['deleted_count' => $deleted_count],
                null
            );
            
            $_SESSION['message'] = "All quiz questions deleted successfully! ($deleted_count questions removed)";
        } else {
            $_SESSION['error'] = "Error deleting quiz questions: " . mysqli_error($con);
        }

        header("Location: manage-quizes.php?subject=$subject_name&level=$level");
        exit();
    }
    ...
    if (isset($_POST['add_quiz'])) {
        ...
        // Insert the quiz
        $query = "INSERT INTO quizes (subject_name, quiz_level, category, question, answer_a, answer_b, answer_c, answer_d, correct_answer_number) 
                  VALUES ('$subject_name', '$quiz_level', '$category', '$question', '$answer_a', '$answer_b', '$answer_c', '$answer_d', '$correct_answer')";
        ...
        header("Location: manage-quizes.php?subject=" . $subject_name . "&level=" . $current_level);
        exit();
    }
    ...
    if (isset($_POST['edit_quiz'])) {
        ...
        $old_query = "SELECT * FROM quizes WHERE id = '$quiz_id'";
    ...
    if (isset($_POST['delete_quiz'])) {
        ...
        if ($old_data) {
            // Delete the quiz
            $query = "DELETE FROM quizes WHERE id = '$quiz_id'";
...
// Fetch quizzes for current subject with level filter (admin sees all)
$quizzes = [];
$available_levels = [];
if ($current_subject && in_array($current_subject, $all_subjects)) {
    // Build query with level filter - no teacher_id restriction for admin
    $query = "SELECT * FROM quizes WHERE subject_name = '$current_subject'";
    ...
    $level_query = "SELECT DISTINCT quiz_level FROM quizes WHERE subject_name = '$current_subject' ORDER BY quiz_level";
...
<head>
    ...
    <style>
        ...
        .bg-science {
            background-color: #1E7D4E !important;
        }
        ...
        <div class="content-wrapper">
            ...
            <div class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1 class="m-0 dashboard-title">Manage In-game Quizes
                                <span class="admin-badge">EDUCATOR</span>
                            </h1>
                            <p class="text-muted mb-0">
                                <i class="fas fa-tasks mr-1"></i>
                                Full access to all subjects and questions
                            </p>
                        </div>
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-right">
                                <li class="breadcrumb-item"><a href="educ-dashboard.php">Home</a></li>
                            ...
                            <div class="card-body">
                                ...
                                <?php else: ?>
                                    <div class="table-responsive">
                                        <table id="quizesTable" class="table table-striped table-hover">
                                            <thead>
                                                <tr>
                                                    <th>ID</th>
                                                    <th>Question</th>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <small class="text-muted">
                                                                <?php echo $quiz['teacher_id'] ? 'Teacher #' . $quiz['teacher_id'] : 'System'; ?>
                                                            </small>
                                                        </td>
                                                        <td>
                            ...
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div id="edit-english-category" class="category-container" style="display: none;">
                                        <select name="category" class="form-control category-select" disabled>
                                            <option value="">Select Category</option>
                                            <?php foreach (getCategoriesBySubject('english') as $cat): ?>
                                    ...
                                    <div id="edit-math-category" class="category-container" style="display: none;">
                                        <select name="category" class="form-control category-select" disabled>
                                            <option value="">Select Category</option>
                                            <?php foreach (getCategoriesBySubject('math') as $cat): ?>
                                                <option value="<?php echo htmlspecialchars($cat); ?>"><?php echo htmlspecialchars($cat); ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    ...
                                    <div id="edit-filipino-category" class="category-container" style="display: none;">
                                        <select name="category" class="form-control category-select" disabled>
                                            <option value="">Select Category</option>
                                            <?php foreach (getCategoriesBySubject('filipino') as $cat): ?>
                                                <option value="<?php echo htmlspecialchars($cat); ?>"><?php echo htmlspecialchars($cat); ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    ...
                                    <div id="edit-ap-category" class="category-container" style="display: none;">
                                        <select name="category" class="form-control category-select" disabled>
                                            <option value="">Select Category</option>
                                            <?php foreach (getCategoriesBySubject('ap') as $cat): ?>
                                                <option value="<?php echo htmlspecialchars($cat); ?>"><?php echo htmlspecialchars($cat); ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    ...
                                    <div id="edit-science-category" class="category-container" style="display: none;">
                                        <select name="category" class="form-control category-select" disabled>
                                            <option value="">Select Category</option>
                                            <?php foreach (getCategoriesBySubject('science') as $cat): ?>
                                                <option value="<?php echo htmlspecialchars($cat); ?>"><?php echo htmlspecialchars($cat); ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
...
<div class="modal fade" id="deleteAllQuizModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST">
                <div class="modal-header bg-danger text-white">
                    <h5 class="modal-title">Delete All Quiz Questions</h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    ...
                    <ul>
                        <li><strong>Subject:</strong> <?php echo $subject_names[$current_subject]; ?></li>
                        <li><strong>Level:</strong> 
                            <?php echo ($current_level === 'all') ? 'ALL Levels' : 'Level ' . $current_level; ?>
                        </li>
                    </ul>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                    <button type="submit" name="delete_all_quiz" class="btn btn-danger">
                        Delete All
                    </button>
                </div>
            </form>
        ...
        <script>
            /**
             * Reusable function to setup category toggle for a modal
             * @param {string} modalId - ID of the modal (addQuizModal or editQuizModal)
             * @param {string} prefix - Prefix for category containers (add or edit)
             */
            function setupCategoryToggle(modalId, prefix) {
                ...
                const subjectSelect = modal.querySelector(`#${prefix}-subject-select`);
                if (!subjectSelect) {
                    console.warn(`Subject select for ${prefix} not found`);
                    return;
                }
                
                // Get all category containers within this modal
                ...
                subjectSelect.addEventListener('change', function() {
                    ...
                    // Show and enable the selected subject's category container
                    if (selectedSubject) {
                        const targetContainer = modal.querySelector(`#${prefix}-${selectedSubject}-category`);
                        if (targetContainer) {
                ...
                // Trigger change event on modal show to handle preselected subjects
            ...
            // Initialize category toggles for both modals
            document.addEventListener('DOMContentLoaded', function() {
                setupCategoryToggle('addQuizModal', 'add');
                setupCategoryToggle('editQuizModal', 'edit');
                
                // Trigger initial category display for Add modal if subject is preselected
                const addSubjectSelect = document.getElementById('add-subject-select');
                if (addSubjectSelect && addSubjectSelect.value) {
                    addSubjectSelect.dispatchEvent(new Event('change'));
                }
            });
            ...
            document.querySelectorAll('.btn-edit').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    editSubjectSelect.value = quiz.subject_name;
                    ...
                    editSubjectSelect.dispatchEvent(new Event('change'));
                    
                    // Set category value after a brief delay to ensure dropdown is visible
                    setTimeout(function() {
                        const categoryContainer = document.querySelector(`#edit-${quiz.subject_name}-category`);
